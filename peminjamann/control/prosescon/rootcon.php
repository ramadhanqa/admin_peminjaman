...
class con{
	...
	function addpeminjaman($con,$id_peminjaman,$tgl_peminjaman,$tgl_kembali,$nama,$id_kat_produk,$id_produk,$alamat,$no_telp,$email,$harga,$keterangan,$transaksi,$deskripsi,$denda){ $tt=date('Y-m-d H:i:s');
		$sql=mysqli_query($con,"INSERT INTO peminjaman set id_peminjaman='$id_peminjaman',tgl_peminjaman='$tt',tgl_kembali='$tgl_kembali',nama='$nama',id_kat_produk='$id_kat_produk',id_produk='$id_produk',alamat='$alamat',no_telp='$no_telp',email='$email',harga ='$harga',keterangan ='$keterangan',transaksi='$transaksi',deskripsi='$deskripsi',denda='$denda' ");
		if($sql){
			header('location:../main?thisaction='.base64_encode('peminjaman').'');
		}else{
			echo "Gagal";
		}
	}
	...
	function editpeminjaman($con,$id_peminjaman,$tgl_peminjaman,$tgl_kembali,$nama,$alamat,$no_telp,$email,$keterangan,$transaksi,$deskripsi,$denda,$harga){  $tt=date('Y-m-d H:i:s');
		$sql=mysqli_query($con,"UPDATE peminjaman SET tgl_kembali = '$tgl_kembali',
		nama = '$nama', keterangan = 'sudah kembali', denda = '$denda',harga='$harga',transaksi='$transaksi', deskripsi='$deskripsi' WHERE id_peminjaman = '$id_peminjaman' ");
		header('location:../main?thisaction='.base64_encode('pengembalian').'');
	}
